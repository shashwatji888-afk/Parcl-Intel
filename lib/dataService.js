...
export async function fetchLiveMarketData() {
  ...
  try {
    ...
    return data.map((item) => {
      ...
      const price1y = (parseFloat(item.price_change_1y || 0) * 100).toFixed(1);
      const cashPct = (parseFloat(item.fin_pct_all_cash || 0.4) * 100).toFixed(0);
      ...
      return {
        ...
        priceCutsPct,
        unrealizedLoss,
        priceChange1y: (parseFloat(price1y) >= 0 ? '+' : '') + price1y + '%',
        cashPct: cashPct + '%',
        singleFamilyPct: (parseFloat(item.pct_supply_single_family || 0.8) * 100).toFixed(0) + '%',
        condoPct: (parseFloat(item.pct_supply_condo || 0.1) * 100).toFixed(0) + '%',
        newConstrPct: (parseFloat(item.pct_supply_new_construction || 0.1) * 100).toFixed(0) + '%',
        hottest: 'SFR · MSI ' + msi.toFixed(2)
      };
    });
  } catch (err) {
